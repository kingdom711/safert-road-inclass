...
- [x] **Backend 프로젝트 구조화**
    ...
    - [x] `build.gradle` 의존성 점검 (Lombok, JPA, Web, Validation, Security) ✅
    - [x] DB 연결 설정 (`application.properties` - MySQL/H2) ✅
- [x] **Frontend 프로젝트 초기화**
    - [x] Vite + React + TS 생성 (완료)
    - [x] TailwindCSS 설정 및 기본 테마 구성 (완료)
    - [x] Axios 및 폴더 구조(`features`, `components`, `pages`) 세팅 (완료)
- [x] **Git 환경 구성**
    - [x] `.gitignore` 점검 (Backend/Frontend 통합) ✅
    - [ ] Branch 전략 수립 (main, develop, feature/*)

## 3. 완료 조건 (Acceptance Criteria)
- [x] Backend 애플리케이션이 에러 없이 구동되어야 한다. ✅ (2025-12-06)
- [x] Frontend 애플리케이션이 구동되고 "Hello World"가 보여야 한다.
- [x] DB 스키마 생성을 위한 설정이 완료되어야 한다. ✅ (H2/MySQL 프로파일 구성)
