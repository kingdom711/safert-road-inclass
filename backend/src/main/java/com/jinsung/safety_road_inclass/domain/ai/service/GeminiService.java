...
public class GeminiService {
    ...
    private String buildUserPrompt(String situationText, String workType, 
        ...
        if (workType != null || location != null || workerCount != null || currentTask != null) {
            prompt.append("추가 정보:\n");
            if (workType != null) prompt.append("- 작업 유형: ").append(workType).append("\n");
            if (location != null) prompt.append("- 작업 위치: ").append(location).append("\n");
            if (workerCount != null) prompt.append("- 작업자 수: ").append(workerCount).append("명\n");
            if (currentTask != null) prompt.append("- 현재 작업: ").append(currentTask).append("\n");
        }
    ...
    private GeminiResponse callGeminiApi(String userPrompt) {
        ...
        log.info("[Gemini API 요청] URL: {}", geminiConfig.getUrl());
        log.debug("[Gemini API 요청] Prompt 길이: {} chars", userPrompt.length());
    ...
    private GeminiAnalysisResult createFallbackResponse(String situationText) {
        ...
        } else if (lowerText.contains("화기") || lowerText.contains("용접") || lowerText.contains("가연") || lowerText.contains("화재")) {
